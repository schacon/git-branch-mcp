...
impl CommitMessageFormatter {
    ...
    pub fn wrap_line(line: &str, leading: Option<String>, indent: Option<usize>) -> String {
                ...
                while x < lines.len() {
                    ...
                    if line.starts_with("```") {
                        ...
                        if is_list_item || is_quote {
                            let leading_spaces = line.len() - line.trim_start().len();
                            if is_list_item {
                                result.push_str(&Self::wrap_line(line, None, Some(4)));
                            } else {
                                result.push_str(&Self::wrap_line(
                                    line,
                                    Some("> ".to_string()),
                                    Some(leading_spaces),
                                ));
...
mod tests {
    ...
    fn test_no_message_roundtrip() {
        ...
        let formatted = CommitMessageFormatter::format_for_commit(original.clone());
        let parsed = CommitMessageFormatter::parse_for_ui(&formatted);
...
be wrapped at word boundaries."#;
        ...
        let formatted_again = CommitMessageFormatter::format_for_commit(parsed);
...
Reviewed-by: Alice Smith <alice.smith@example.com>"#.to_string();
        let formatted = CommitMessageFormatter::format_for_commit(message.clone());
